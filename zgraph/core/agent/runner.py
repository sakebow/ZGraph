    ...
    def run(
        ...
        *,
        ...
        if messages:
            ...
            if content == "" and isinstance(last, dict):
                content = str(last.get("content", "") or "")
            # 从最后一条 AIMessage 提取 reasoning_content（thinking 模型）
            extra = getattr(last, "additional_kwargs", None)
            if isinstance(extra, dict):
                reasoning = str(extra.get("reasoning_content", "") or "")
        if not content:
            content = str(result) if result is not None else ""
        output = AgentResult(content=content, reasoning_content=reasoning)
...
def _write_conversation(
    ...
    *,
    ...
    参数:
        ...
        user_input: 用户输入的原始文本（str）。
        output: 智能体返回的 AgentResult（含 content / reasoning_content）。
