...
class ZGraphRuntime:
    ...
    def _run_unprotected(self, request: dict[str, Any], workspace: RunWorkspace) -> dict[str, Any]:
        ...
        self.logger.info(
            "run_id=%s stage=execute:end elapsed_ms=%.2f chars=%s",
            workspace.run_id,
    ...
    def _execute(
        ...
        if self.settings.offline or not self.settings.api_key:
            ...
            return self._offline_execute(user_input, workspace, state)
        try:
            ...
            return self._offline_execute(user_input, workspace, state, error=str(exc))
        finally:
            self.logger.info(
